...
echo.
echo The server is up, access it by using the ip 0.0.0.0:33330, also it is
echo a local bungee, and it uses a custom coded plugin for exploiting (credits to wrrulos).
echo.
echo Press enter to continue:
cd ../
pause>nul
...
cd dbsearcher && del /q found_password.txt
...
cls
echo.
ping -l 100 dev.%glue% > nul
if errorlevel 0 (
   echo Subdomain found: dev.%glue% && ping dev.%glue% | for /f "skip=3 tokens=*" %%a in ('findstr Reply') do @echo %%a && echo.
)
ping -l 100 test.%glue% > nul
if errorlevel 0 (
   echo Subdomain found: test.%glue% && ping test.%glue% | for /f "skip=3 tokens=*" %%a in ('findstr Reply') do @echo %%a && echo.
)
ping -l 100 ovh.%glue% > nul
if errorlevel 0 (
   echo Subdomain found: ovh.%glue% && ping ovh.%glue% | for /f "skip=3 tokens=*" %%a in ('findstr Reply') do @echo %%a && echo.
)
ping -l 100 staff.%glue% > nul
if errorlevel 0 (
   echo Subdomain found: staff.%glue% && ping staff.%glue% | for /f "skip=3 tokens=*" %%a in ('findstr Reply') do @echo %%a && echo.
)
ping -l 100 jogar.%glue% > nul
if errorlevel 0 (
   echo Subdomain found: jogar.%glue% && ping jogar.%glue% | for /f "skip=3 tokens=*" %%a in ('findstr Reply') do @echo %%a && echo.
)
ping -l 100 play.%glue% > nul
if errorlevel 0 (
   echo Subdomain found: play.%glue% && ping play.%glue% | for /f "skip=3 tokens=*" %%a in ('findstr Reply') do @echo %%a && echo..
)
ping -l 100 mc.%glue% > nul
if errorlevel 0 (
   echo Subdomain found: mc.%glue% && ping mc.%glue% | for /f "skip=3 tokens=*" %%a in ('findstr Reply') do @echo %%a && echo.
)
ping -l www.%glue% > nul
if errorlevel 0 (
   echo Subdomain found: www.%glue% && ping www.%glue% | for /f "skip=3 tokens=*" %%a in ('findstr Reply') do @echo %%a && echo.
)
ping -l 100 build.%glue% > nul
if errorlevel 0 (
   echo Subdomain found: build.%glue% && ping build.%glue% | for /f "skip=3 tokens=*" %%a in ('findstr Reply') do @echo %%a && echo.
)
ping -l 100 store.%glue% > nul
if errorlevel 0 (
   echo Subdomain found: store.%glue% && ping store.%glue% | for /f "skip=3 tokens=*" %%a in ('findstr Reply') do @echo %%a && echo.
)
ping -l 100 na.%glue% > nul
if errorlevel 0 (
   echo Subdomain found: na.%glue% && ping na.%glue% | for /f "skip=3 tokens=*" %%a in ('findstr Reply') do @echo %%a && echo.
)
ping -l 100 as.%glue% > nul
if errorlevel 0 (
   echo Subdomain found: as.%glue% && ping as.%glue% | for /f "skip=3 tokens=*" %%a in ('findstr Reply') do @echo %%a && echo.
)
ping -l 100 eu.%glue% > nul
if errorlevel 0 (
   echo Subdomain found: eu.%glue% && ping eu.%glue% | for /f "skip=3 tokens=*" %%a in ('findstr Reply') do @echo %%a && echo.
)
ping -l 100 node01.%glue% > nul
if errorlevel 0 (
   echo Subdomain found: node01.%glue% && ping node01.%glue% | for /f "skip=3 tokens=*" %%a in ('findstr Reply') do @echo %%a && echo.
)
ping -l 100 node02.%glue% > nul
if errorlevel 0 (
   echo Subdomain found: node02.%glue% && ping node02.%glue% | for /f "skip=3 tokens=*" %%a in ('findstr Reply') do @echo %%a && echo.
)
ping -l 100 node03.%glue% > nul
if errorlevel 0 (
   echo Subdomain found: node03.%glue% && ping node03.%glue% | for /f "skip=3 tokens=*" %%a in ('findstr Reply') do @echo %%a && echo.
)
ping -l 100 node04.%glue% > nul
if errorlevel 0 (
   echo Subdomain found: node04.%glue% && ping node04.%glue% | for /f "skip=3 tokens=*" %%a in ('findstr Reply') do @echo %%a && echo.
)
ping -l 100 node05.%glue% > nul
if errorlevel 0 (
   echo Subdomain found: node05.%glue% && ping node05.%glue% | for /f "skip=3 tokens=*" %%a in ('findstr Reply') do @echo %%a && echo.
)
ping -l 100 vote.%glue% > nul
if errorlevel 0 (
   echo Subdomain found: vote.%glue% && ping vote.%glue% | for /f "skip=3 tokens=*" %%a in ('findstr Reply') do @echo %%a && echo.
)
ping -l 100 vote1.%glue% > nul
if errorlevel 0 (
   echo Subdomain found: vote1.%glue% && ping vote1.%glue% | for /f "skip=3 tokens=*" %%a in ('findstr Reply') do @echo %%a && echo.
)
ping -l 100 vote2.%glue% > nul
if errorlevel 0 (
   echo Subdomain found: vote2.%glue% && ping vote2.%glue% | for /f "skip=3 tokens=*" %%a in ('findstr Reply') do @echo %%a && echo.
)
ping -l 100 vote3.%glue% > nul
if errorlevel 0 (
   echo Subdomain found: vote3.%glue% && ping vote3.%glue% | for /f "skip=3 tokens=*" %%a in ('findstr Reply') do @echo %%a && echo.
)
ping -l 100 vote4.%glue% > nul
if errorlevel 0 (
   echo Subdomain found: vote4.%glue% && ping vote4.%glue% | for /f "skip=3 tokens=*" %%a in ('findstr Reply') do @echo %%a && echo.
)
ping -l 100 vote5.%glue% > nul
if errorlevel 0 (
   echo Subdomain found: vote5.%glue% && ping vote5.%glue% | for /f "skip=3 tokens=*" %%a in ('findstr Reply') do @echo %%a && echo.
)
echo.
echo Scan finished.
echo Press enter to continue.
